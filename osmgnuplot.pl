...
my ($montage_filename, $width, $height) = make_montage(\@lat, \@lon, $zoom);

# Get the mapping between montage pixels and lat/lon. This assumes that both are
# linear. In truth this is only linear for longitude. Latitude is linear-enough
# (I interpolate the ends) when I look at a small range of values. This becomes
# wrong when I query huge areas, and hopefully nobody is doing that
my ($centerx, $centery, $dx, $dy) =
  get_pixel_mapping($width, $height, \@lat, \@lon);
...
sub make_montage
{
    ...
    # And the tile indices of the corners. This tells me which tiles need to be
    # downloaded. The tiles cover a larger area than what I care about
    my @tilex =         map { lon2tilex($_, $zoom ) } @$lon;
    my @tiley = reverse map { lat2tiley($_, $zoom ) } @$lat; # lat pixels map in opposite order
    ...
    my @lon_px = map { int( 0.5 + lon2tilex_pixels($_, $zoom) - 256*$tilex[0] ) }         @lon;
    my @lat_px = map { int( 0.5 + lat2tiley_pixels($_, $zoom) - 256*$tiley[0] ) } reverse @lat;
...
sub get_pixel_mapping
{
    ...
    # image and the lat/lon positions of the corners
...
sub lon2tilex
{
    return int(lon2tilex_pixels(@_) / 256);
}
sub lat2tiley_pixels
{
    my ($lat, $zoom) = @_;
    my $lata = $lat*$pi/180;
    ...
    my $s = sin($lata);
    my $c = cos($lata);
    return (1 - log( ($s + 1.)/$c )/$pi)/2 * 2**$zoom * 256;
}
sub lat2tiley
{
    return int(lat2tiley_pixels(@_) / 256);
}
sub tile2path
{
